...
import { DEFAULT_PLOT_ID, MASTERPLAN_VIEWBOX, PLOT_PHOTOS, PLOTS, SELLABLE_PLOTS, formatArea, formatPLN } from './plotData';
    ...
    <div className="masterplan-wrap">
      <div className="masterplan-stage">
        <svg className="masterplan-svg" viewBox={MASTERPLAN_VIEWBOX} preserveAspectRatio="xMidYMid meet" role="img" aria-label="Interaktywna mapa działek Warszawska Park">
          <defs>
            <linearGradient id="mapWash" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#143f2f" stopOpacity=".76"/><stop offset=".6" stopColor="#486a45" stopOpacity=".42"/><stop offset="1" stopColor="#203c2d" stopOpacity=".72"/></linearGradient>
            <filter id="softBg"><feGaussianBlur stdDeviation="2.2"/></filter>
            <filter id="plotGlow" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="4" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
            <linearGradient id="fieldLight" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#87a66a" stopOpacity=".2"/><stop offset="1" stopColor="#243f2d" stopOpacity=".28"/></linearGradient>
          </defs>
          <image href="/assets/hero-bg.jpg" x="0" y="0" width="1024" height="518" preserveAspectRatio="xMidYMid slice" opacity=".62" filter="url(#softBg)"/>
          <rect x="0" y="0" width="1024" height="518" fill="url(#mapWash)"/>
          <rect x="0" y="118" width="1024" height="265" fill="url(#fieldLight)" opacity=".7"/>
          {/* Delikatna geometria dróg pod obrysami — porządkuje plan bez udawania ortofotomapy. */}
          <path d="M 20 248 C 250 247 475 249 1010 250" fill="none" stroke="#a7a18c" strokeWidth="22" strokeOpacity=".44"/>
          <path d="M 20 366 C 215 363 430 365 1010 368" fill="none" stroke="#868475" strokeWidth="24" strokeOpacity=".28"/>
          <path d="M 21 248 C 250 247 475 249 1010 250" fill="none" stroke="#d8d2bb" strokeWidth="1" strokeOpacity=".58"/>
          <path d="M 20 366 C 215 363 430 365 1010 368" fill="none" stroke="#d8d2bb" strokeWidth="1" strokeOpacity=".36"/>
          ...
          {PLOTS.map(plot => {
            ...
            const common = plot.status === 'common';
            const fill = common
              ? (plot.klasa === 'PARK' ? 'rgba(85,118,72,.34)' : plot.klasa === 'DROGOWA' ? 'rgba(149,143,124,.38)' : 'rgba(129,137,111,.22)')
              : plot.status === 'stage2'
                ? (selected || hovered ? 'rgba(194,157,87,.62)' : 'rgba(190,158,101,.18)')
                : (selected || hovered ? 'rgba(77,119,76,.76)' : 'rgba(112,141,90,.20)');
            const stroke = selected ? '#f5e8bd' : hovered ? '#e5c36f' : common ? 'rgba(240,235,213,.46)' : 'rgba(255,255,255,.62)';
            return <g key={plot.id}
              data-plot={plot.id}
              className={`masterplot masterplot--${statusClass(plot)} ${selected ? 'is-selected' : ''} ${mutedByType ? 'is-muted' : ''}`}
              role={interactive ? 'button' : 'img'}
              tabIndex={interactive ? 0 : -1}
              aria-label={interactive ? `Działka ${plot.id}, ${plot.size} metrów kwadratowych, ${plot.statusLabel}` : `${plot.id}, teren wspólny`}
              onMouseEnter={() => interactive && onHover(plot.id)}
              onMouseLeave={() => onHover(null)}
              onFocus={() => interactive && onHover(plot.id)}
              onBlur={() => onHover(null)}
              onClick={() => interactive && onSelect(plot.id)}
              onKeyDown={e => { if(interactive && (e.key === 'Enter' || e.key === ' ')){ e.preventDefault(); onSelect(plot.id); } }}>
              <path d={plot.d} fill={fill} stroke={stroke} strokeWidth={selected ? 2.5 : hovered ? 2 : 1.1} vectorEffect="non-scaling-stroke" filter={selected ? 'url(#plotGlow)' : undefined}/>
              {plot.sellable && <>
                <circle cx={plot.cx} cy={plot.cy} r={selected ? 11 : 9.4} fill={selected ? '#f9f4e5' : 'rgba(251,248,238,.91)'} stroke={selected ? '#d2a94e' : 'rgba(12,47,35,.25)'} strokeWidth="1"/>
                <text x={plot.cx} y={plot.cy + 2.9} textAnchor="middle" className="masterplot-label">{plot.id}</text>
              </>}
            </g>;
          })}
        </svg>
        <Compass/>
        <div className="masterplan-caption">PLAN POGLĄDOWY · KLIKNIJ DZIAŁKĘ</div>
        <MapLegend/>
      </div>
    </div>
  );
